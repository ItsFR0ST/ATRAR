...
module.exports.run = async (bot, message, args) => {
    ...
    let dare = new Discord.MessageEmbed()
    ...
    if (!user) return message.channel.send(dare)

        let darec = new Discord.MessageEmbed()
            .setAuthor(`${message.author.username}`, message.author.displayAvatarURL({ dynamic: true }))
        .setDescription(`${message.author} Choosed ${user} to do a **Dare** \n \n ${replies[result]}`)
            .setThumbnail(user.user.displayAvatarURL({ dynamic: true }))
            .setColor("RANDOM")
            .setFooter(`if you want to add more dares go to the support server`)
            .setTimestamp()

    if (user) return message.channel.send(darec)
