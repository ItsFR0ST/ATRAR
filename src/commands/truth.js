...
module.exports.run = async (bot, message, args) => {
    ...
    let truth = new Discord.MessageEmbed()
    ...
    if (!user) return message.channel.send(truth)    



        let truthc = new Discord.MessageEmbed()
            .setAuthor(`${message.author.username}`, message.author.displayAvatarURL({ dynamic: true }))
            .setDescription(`${message.author} Choosed ${user} to say a **Truth**\n \n ${replies[result]}`)
            .setThumbnail(user.user.displayAvatarURL({ dynamic: true }))
            .setColor("RANDOM")
            .setFooter(`if you want to add more truths go to the support server`)
            .setTimestamp()

    if (user) return message.channel.send(truthc)
    }
